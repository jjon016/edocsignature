...
it('retrieves documents with different status', async () => {
  //create doc data
  const userId = mongoose.Types.ObjectId().toHexString();
  const docOneId = mongoose.Types.ObjectId().toHexString();
  ...
  const response = await request(app)
    ...
    .send()
    .expect(200);
  expect(response.body.docs.length).toEqual(2);
});

it('passes empty array if no docs found', async () => {
